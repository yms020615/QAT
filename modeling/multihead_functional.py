...
def _scaled_dot_product_attention(
    ...
    dropout_p: float = 0.0,
    relative_position_bias: Tensor = None,
) -> Tuple[Tensor, Tensor]:
    r"""
    ...
    if relative_position_bias is not None:
        attn = attn + relative_position_bias[:, :attn.shape[1], :attn.shape[2]]
...
def multi_head_attention_forward(
    ...
    matched_index: Optional[Tensor] = None,
    rel_position_bias: Tensor = None,
    num_lmtokens: int = None,
    rpe_2: bool = False,
    ...
    # relative position embedding
    if matched_index is not None and matched_index.size(-1) == 2:
        matched_1 = matched_index[:,:,0].view(-1).contiguous()
        matched_2 = matched_index[:,:,1].view(-1).contiguous()
        idx = torch.arange(matched_index.size(0)*matched_index.size(1), device=matched_1.device)
        val = torch.ones(idx.shape[0], device=idx.device)
        m1 = torch.stack((idx, matched_1))
        m2 = torch.stack((idx, matched_2))
        s1 = torch.sparse_coo_tensor(m1, val, [idx.size(0), num_lmtokens], device=idx.device).to_dense()
        s2 = torch.sparse_coo_tensor(m2, val, [idx.size(0), num_lmtokens], device=idx.device).to_dense()
        # s = s1 + s2
        s = torch.logical_or(s1, s2).float()
        d1 = rearrange(s, '(b e) l -> b l e', b=bsz)
        d2 = rearrange(s, '(b e) l -> b e l', b=bsz)
        relative_pos_map1 = pad(d1, (d1.size(1), 0))
        relative_pos_map2 = pad(d2, (0, d2.size(1)))
        mapping = torch.cat([relative_pos_map1, relative_pos_map2], dim=1)
        if rpe_2:
            rpe1 = rel_position_bias[:,0].reshape(1, -1, 1, 1)
            rpe2 = rel_position_bias[:,1].reshape(1, -1, 1, 1)
            rpe_map1 = relative_pos_map1.unsqueeze(1) * rpe1
            rpe_map2 = relative_pos_map2.unsqueeze(1) * rpe2
            relative_position_bias = torch.cat([rpe_map1, rpe_map2], dim=2)
            relative_position_bias = relative_position_bias.reshape(bsz*num_heads, relative_position_bias.shape[-2], relative_position_bias.shape[-1])
        else:
            rpe = torch.cat([relative_pos_map1, relative_pos_map2], dim=1)
            relative_position_bias = (rpe.unsqueeze(1) * rel_position_bias.reshape(1, -1, 1, 1)).reshape(bsz*num_heads, rpe.shape[-2], rpe.shape[-1])
    elif matched_index is not None and matched_index.size(-1) == 1:
        matched_1 = matched_index[:,:,0].view(-1).contiguous()
        idx = torch.arange(matched_index.size(0)*matched_index.size(1), device=matched_1.device)
        val = torch.ones(idx.shape[0], device=idx.device)
        m1 = torch.stack((idx, matched_1))
        s = torch.sparse_coo_tensor(m1, val, [idx.size(0), num_lmtokens], device=idx.device).to_dense()
        # s = s1 + s2
        d1 = rearrange(s, '(b e) l -> b l e', b=bsz)
        d2 = rearrange(s, '(b e) l -> b e l', b=bsz)
        relative_pos_map1 = pad(d1, (d1.size(1), 0))
    ...
    attn_output, attn_output_weights, ori_attn_weights = _scaled_dot_product_attention(q, k, v, attn_mask, dropout_p, relative_position_bias)
    ...
    if need_weights:
        ...
        ori_attn_weights = ori_attn_weights.view(bsz, num_heads, tgt_len, src_len)
        return attn_output, attn_output_weights.sum(dim=1) / num_heads, ori_attn_weights
    else:
