...
import numpy as np
from transformers import (GPT2_PRETRAINED_CONFIG_ARCHIVE_MAP, BERT_PRETRAINED_CONFIG_ARCHIVE_MAP,
                          XLNET_PRETRAINED_CONFIG_ARCHIVE_MAP, ROBERTA_PRETRAINED_CONFIG_ARCHIVE_MAP, GPT2LMHeadModel)
try:
...
MODEL_CLASS_TO_NAME = {
    'gpt2': list(GPT2_PRETRAINED_CONFIG_ARCHIVE_MAP.keys()),
...
class TextEncoder(nn.Module):
    ...
    def __init__(self, model_name, output_token_states=False, from_checkpoint=None, **kwargs):
        ...
        if self.model_type in ('lstm', ):
            ...
            self.sent_dim = self.module.config.n_embd if self.model_type in ('gpt2',) else self.module.config.hidden_size
    ...
    def forward(self, *inputs, layer_id=-1):
        ...
        # elif self.model_type in ('gpt2',):
        #     cls_token_ids = cls_token_ids.view(-1).unsqueeze(-1).unsqueeze(-1).expand(-1, 1, hidden_states.size(-1))
        #     sent_vecs = hidden_states[:, :, -1, :].gather(dim=2, index=cls_token_ids).squeeze(1)
        elif self.model_type in ('xlnet',):
            sent_vecs = hidden_states[:, -1]
        elif self.model_type in ('albert',):
            if self.output_token_states:
