...
import numpy as np
import torch
from transformers import (GPT2Tokenizer, AutoTokenizer, BertTokenizer, XLNetTokenizer, RobertaTokenizer)
try:
...
class MultiGPUSparseAdjDataBatchGenerator(object):
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            metapath_feature = self.metapath_data
            ...
            yield tuple([batch_qids, batch_labels, *batch_tensors0, *batch_lists0, *batch_tensors1, *batch_lists1, metapath_feature, edge_index, edge_type])
...
def load_sparse_adj_data_with_contextnode(adj_pk_path, max_node_num, num_choice, args):
    mrn_cache_path = adj_pk_path + '.mrn'+ '1hop' + str(max_node_num) + '.loaded_cache'
    if max_node_num == 200:
...
def load_bert_xlnet_roberta_input_tensors(statement_jsonl_path, model_type, model_name, max_seq_length, pretrained=None):
    ...
    def convert_examples_to_features(examples, label_list, max_seq_length,
                                     ...
                                     sep_token='[SEP]',
                                     # sep_token='_delimiter_',
                                     # end_token='_classify_',
        ...
        for ex_index, example in enumerate(examples):
            ...
            for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
                ...
                special_token_id = tokenizer.convert_tokens_to_ids([cls_token, sep_token]) #, end_token])
    ...
    try:
        tokenizer_class = {'gpt2': GPT2Tokenizer, 'bert': BertTokenizer, 'xlnet': XLNetTokenizer, 'roberta': RobertaTokenizer, 'albert': AlbertTokenizer}.get(model_type)
    except:
        tokenizer_class = {'gpt2': GPT2Tokenizer, 'bert': BertTokenizer, 'xlnet': XLNetTokenizer, 'roberta': RobertaTokenizer}.get(model_type)
...
def load_input_tensors(input_jsonl_path, model_type, model_name, max_seq_length, pretrained):
    ...
    elif model_type in ('bert', 'xlnet', 'roberta', 'albert', 'aristo-roberta', 'gpt2'):
