...
class RobertaForMaskedLMwithLoss(RobertaForMaskedLM):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, masked_lm_labels=None):
        ...
        outputs = self.transformer(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, position_ids=position_ids, head_mask=head_mask)
...
def get_LM_score(cids, question):
    ...
    while cur_idx < n_cids:
        ...
        with torch.no_grad():
            ...
            batch_scores = np.repeat(loss.detach().cpu().numpy(), batch_size)
...
if __name__ == '__main__':
    generate_adj_data_from_grounded_concepts__use_LM('./data/csqa/grounded/train.grounded.jsonl',
                                                 ...
                                                 './data/csqa/graph/train.pk', 40)
    generate_adj_data_from_grounded_concepts__use_LM('./data/csqa/grounded/dev.grounded.jsonl',
                                                 './data/cpnet/conceptnet.en.pruned.graph',
                                                 './data/cpnet/concept.txt',
                                                 './data/csqa/graph/dev.pk', 40)
    #generate_adj_data_from_grounded_concepts__use_LM('./data/csqa/grounded/test.grounded.jsonl',
    #                                             './data/cpnet/conceptnet.en.pruned.graph',
    #                                             './data/cpnet/concept.txt',
    #                                             './data/csqa/graph/test.pk', 40)
