...
class CQAttention(torch.nn.Module):
    def __init__(self, block_hidden_dim, qk_bias=True):
        super().__init__()
        self.q_linear = nn.Linear(block_hidden_dim, block_hidden_dim, bias=qk_bias)
        self.k_linear = nn.Linear(block_hidden_dim, block_hidden_dim, bias=qk_bias)

    
    def forward(self, query, key, query_mask, key_mask, return_att=False):
        query = self.q_linear(query) #(B, N, D)
        key = self.k_linear(key) #(B, M, D)
        attn = query@key.transpose(-2,-1) #(B, N, M)
        attn = self.masked_softmax(attn, key_mask, dim=-1)
        return attn
    def masked_softmax(self, attn, m=None, dim=-1):
        if m is not None:
            mask = (~m).float().clone()
            mask[(~m)] = -torch.inf #(B, M)
