...
class MultiGPUSparseAdjDataBatchGenerator_path(object):
    def __init__(self, args, mode, device0, device1, batch_size, indexes, qids, labels,
...
def load_sparse_adj_data_with_contextnode_with_path(adj_pk_path, max_node_num, num_choice, args):
    cache_path = adj_pk_path +'.loaded_cache'
    use_cache = True
    ...
    if use_cache:
        ...
        with open(cache_path, 'wb') as f:
            pickle.dump([adj_lengths_ori, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type, half_n_rel, metapath_feature, metapath_feature_count], f)
