...
def main():
    ...
    parser.add_argument('--add_nodefeatsim', default='prod', choices=['diff', 'prod'])
...
def train(args):
    ...
    # model, optimizer, scheduler = checkpoint[0], checkpoint[2], checkpoint[3]
    ...
    for epoch_id in range(args.n_epochs):
        ...
        if dev_acc >= best_dev_acc:
            ...
            if args.save_model:
                torch.save([model, args, optimizer.state_dict(), scheduler.state_dict()], model_path)
...
def eval_detail(args):
    ...
    cp_emb = [np.load(path) for path in args.ent_emb_paths]
    ep_emb = torch.tensor(np.concatenate(cp_emb, 1), dtype=torch.float)
    concept_num, concept_dim = cp_emb.size(0), cp_emb.size(1)
    print('| num_concepts: {} |'.format(concept_num))
    ...
    model = LM_QAT(old_args, old_args.encoder, k=old_args.k, n_ntype=4, n_etype=old_args.num_relation,
                   ...
                   pretrained_concetp_emb=cp_emb, concept_in_dim=concept_dim, concept_dim=old_args.transformer_dim, n_concept=concept_num)
    
    model.load_state_dic(model_state.state_dict(), strict=False)
