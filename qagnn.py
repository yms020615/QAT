...
def evaluate_accuracy(eval_set, model):
    ...
    with torch.no_grad():
        for qids, labels, *input_data in tqdm(eval_set):
            ...
            eval_in_target_count += torch.sum(preds < 4).item()
            eval_total += preds.size(0)
    in_target_ratio = eval_in_target_count / eval_total if eval_total > 0 else 0
    return n_correct / n_samples, in_target_ratio
...
def main():
    ...
    parser.add_argument('--train_adj', default=f'data/{args.dataset}/graph/train.pk')
    parser.add_argument('--dev_adj', default=f'data/{args.dataset}/graph/dev.pk')
    parser.add_argument('--test_adj', default=f'data/{args.dataset}/graph/test.pk')
    ...
    # regularization
    parser.add_argument('--dropouti', type=float, default=0.5, help='dropout for embedding layer')
    parser.add_argument('--dropoutg', type=float, default=0.5, help='dropout for GNN layers')
    parser.add_argument('--dropoutf', type=float, default=0.5, help='dropout for fully-connected layers')
...
def train(args):
    ...
    # try:
        for epoch_id in range(args.n_epochs):
            ...
            for qids, labels, *input_data in dataset.train():
                ...
                for a in range(0, bs, args.mini_batch_size):
                    ...
                    with torch.no_grad():
                        ...
                        train_correct += (predictions == labels[a:b]).sum().item()
                        train_total += b - a
                        in_target_count += torch.sum(predictions < 4).item()
                    if args.fp16:
                ...
                if (global_step + 1) % args.log_interval == 0:
                    ...
                    in_target_ratio = in_target_count / train_total
                    print(f'| step {global_step:5} | lr: {scheduler.get_lr()[0]:9.7f} | loss {total_loss:7.4f} | train_acc {train_acc:7.4f} | in_target_ratio {in_target_ratio:7.4f} | ms/batch {ms_per_batch:7.2f} |')
                    total_loss = 0
                    train_correct = 0
                    train_total = 0
                    in_target_count = 0
            ...
            eval_total = 0
            model.eval()
            dev_acc, dev_in_target_ratio = evaluate_accuracy(dataset.dev(), model)
            save_test_preds = args.save_model
            if not save_test_preds:
                test_acc, test_in_target_ratio = evaluate_accuracy(dataset.test(), model) if args.test_statements else 0.0
            else:
                ...
                with open(preds_path, 'w') as f_preds:
                    with torch.no_grad():
                        for qids, labels, *input_data in tqdm(eval_set):
                            ...
                            eval_in_target_count += torch.sum(predictions < 4).item()
                            eval_total += predictions.size(0)
            ...
            print('-' * 71)
            print('| epoch {:3} | step {:5} | dev_acc {:7.4f} | test_acc {:7.4f} | dev_in_target_ratio {:7.4f} | test_in_target_ratio {:7.4f} |'.format(epoch_id, global_step, dev_acc, test_acc, dev_in_target_ratio, test_in_target_ratio))
            print('-' * 71)
            with open(log_path, 'a') as fout:
                fout.write('{},{},{},{}\n'.format(global_step, dev_acc, test_acc, in_target_ratio))
